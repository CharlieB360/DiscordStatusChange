...
function getStatusString(lyrics, time) {
    return `${settings.view.timestamp ? `[${formatSeconds((time / 1000).toFixed(0))}] ` : ""}${settings.view.label ? "Current - " : ""}${lyrics}`
}
function sleep(ms) {
...
function updatePlaybackState() {
    return $.ajax({
        ...
        statusCode: {
            ...
            404: () => { addLog("Got unexpected error!"color: #ff0000;\" href=\"">this</a>. <strong class=\"error\">Error code: 404</strong>", "error"); stopLog = true; stopped = true; errorCount++ },
            502: () => { addLog("Got unexpected error!"color: #ff0000;\" href=\"">this</a>. <strong class=\"error\">Error code: 502</strong>", "error"); stopLog = true; stopped = true; errorCount++ }
        }
    });
}
